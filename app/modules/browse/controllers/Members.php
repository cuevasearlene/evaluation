...
class Members extends MX_Controller
{
    ...
    function load_evaluated_members(){
        ...
        $this->db->where('s.guid',2);
        $this->general->table = 'users as s';
        $this->db->where('s.id IN (SELECT evaluator_id from evaluation_authorize where evaluation_id = "'.$_POST["id"].' and evaluted = 1")', NULL, FALSE);
        $this->db->select(array('s.username', 's.category'));
        ...
        $this->db->where('s.id IN (SELECT evaluator_id from evaluation_authorize where evaluation_id = "'.$_POST["id"].'"  and evaluated = 1)', NULL, FALSE);
        $this->db->where('s.guid',2);
        $total = $this->general->count_all();
