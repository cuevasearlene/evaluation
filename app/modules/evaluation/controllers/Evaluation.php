...
class Evaluation extends MX_Controller
{
	...
	function index()
	{
		...
		$this->viewdata['get_table'] = $this->general->get_table('evaluation as e', '', 
			'e.*, 
		...
		$base_url = site_url('evaluation/index/');
		...
		$this->viewdata['content'] = 'evaluation/table';
	...
	function delete_evaluation(){
		$this->general->blocked_page('evaluation', 'drop');
		$all_post = $this->general->all_post();

		if($all_post->evaluation_id != $all_post->confirmation){
			exit(json_encode(array('status' => 'error', 'message' => 'Invalid Confirmation')));
		}


	    $this->general->delete_table('evaluation', array('id' => $all_post->evaluation_id));
	    $this->general->delete_table('evaluation_attachment', array('evaluation_id' => $all_post->evaluation_id));
	    $this->general->delete_table('evaluation_authorize', array('evaluation_id' => $all_post->evaluation_id));
	    $this->general->delete_table('evaluation_comment', array('evaluation_id' => $all_post->evaluation_id));
	    $this->general->delete_table('evaluation_ratings', array('evaluation_id' => $all_post->evaluation_id));
	 	
	 	$res = $this->general->get_table('question_evaluation', array('evaluation_id' => $all_post->evaluation_id), 'id');
	 	foreach ($res->result() as $vals) {
	 	   $this->general->delete_table('evaluation_summary', array('question_id' => $vals->id));
	 	   $this->general->delete_table('evaluation_summary_other', array('question_id' => $all_post->evaluation_id));
	 	   $this->general->delete_table('question_choices', array('question_eval_id' => $all_post->evaluation_id));
	 	}
	 		exit(json_encode(array('status' => 'sucess', 'message' => 'Successfully Deleted')));
	...
	function upload_file(){
        ...
        $filetype = $this->general->get_table('settings', array('name' => 'filetype'), 'data')->row()->data;
	...
	function retrieve_ratings(){
        ...
        $this->general->column_search = array('username', 'rate', 'created_at');
        $this->general->column_order = array('username', 'rate', 'created_at');

       $this->general->table = 'evaluation_ratings as er';
       $this->db->where('er.evaluation_id', $all_post->id);
       $this->db->order_by('er.created_at', 'desc');
       $this->db->join('users as s', 's.id  = er.uid');
       $this->db->select('s.username, er.rate, er.created_at');
